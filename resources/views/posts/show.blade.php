...
@section('content')
    ...
    <div class="col-md-8 col-md-offset-2 col-xs-12">
      <div class="mainheading">
        ...
        <div class="row post-top-meta">
          <div class="col-md-2">
            <a href="author.html"><img class="author-thumb" src="{{ Storage::url($post['author_photo']) }}"
                alt="{{ $post['author'] }}"></a>
          </div>
          <div class="col-md-10">
            <span class="link-dark text-dark">{{ $post['author'] }}</span>
            <span class="author-description">{{ $post['description'] }}</span>
            <span class="post-date">
              Created at {{ \Carbon\Carbon::parse($post['created_at'])->isoFormat('D MMMM Y') }}
            </span>
            <span class="dot"></span>
            <span class="post-read">
              Last update at {{ \Carbon\Carbon::parse($post['updated_at'])->isoFormat('D MMMM Y') }}</span>
          </div>
        </div>
        <!-- End Top Menta -->
      ...
      <img class="featured-image img-fluid" src="{{ Storage::url($post['image']) }}" alt="{{ $post['title'] }}">
      <!-- End Featured Image -->
      ...
      <!-- Begin Tags -->
      <div class="after-post-tags text-right">
        <a href="{{ route('post.edit', $post['slug']) }}" class="btn btn-warning">Edit</a>
        <a href="javascript:;" id="btn-delete" class="btn btn-danger">Delete</a>
      </div>
      <!-- End Tags -->
  ...
  <form id="form-delete" action="{{ route('post.destroy', $post['slug']) }}" method="post" hidden>
    @csrf
    @method('DELETE')
  </form>
@endsection
